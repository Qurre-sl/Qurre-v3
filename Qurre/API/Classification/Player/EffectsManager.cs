...
public sealed class EffectsManager
{
    ...
    private static readonly AccessTools.FieldRef<PlayerEffectsController, Dictionary<Type, StatusEffectBase>> EffectsByTypeRef =
        AccessTools.FieldRefAccess<PlayerEffectsController, Dictionary<Type, StatusEffectBase>>("_effectsByType");

    public StatusEffectBase? Get(EffectType effect)
    {
        Type type = effect.Type();
        EffectsByTypeRef(Controller).TryGetValue(type, out StatusEffectBase @base);
        return @base;
    }
    ...
    public bool TryGet(EffectType effect, out StatusEffectBase statusEffect)
    {
        ...
        if (EffectsByTypeRef(Controller).TryGetValue(type, out StatusEffectBase @base))
        {
            statusEffect = @base;
            return true;
        }
