...
public static class AudioExtensions
{
        ...
        )
        {
            ...
            while (audioTask.IsRunning)
            {
                foreach (Player? pl in Player.List)
                {
                    ...
                    type.GetField("_syncPos", MimicPointPropertiesBindingFlags)
    ...
    private static readonly Type RpcStateMsgType = typeof(MimicPointController).GetNestedType("RpcStateMsg", BindingFlags.NonPublic);
    private static readonly object RpcStateMsgPlacedByUser = Enum.Parse(RpcStateMsgType, "PlacedByUser");

    private static void DoMimicPointInit(MimicPointController mimicPoint)
    {
        Type type = typeof(MimicPointController);

        type.GetField("_syncMessage", MimicPointPropertiesBindingFlags)
            ?.SetValue(mimicPoint, RpcStateMsgPlacedByUser);

        type.GetField("_syncPos", MimicPointPropertiesBindingFlags)
            ?.SetValue(mimicPoint, new RelativePosition(Vector3.zero));

        type.GetField("_active", MimicPointPropertiesBindingFlags)
