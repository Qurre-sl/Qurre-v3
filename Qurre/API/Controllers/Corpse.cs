...
public class Corpse : NetTransform
{
    ...
    public Corpse(RoleTypeId type, Vector3 position, Quaternion rotation, DamageHandlerBase handler, string nickname)
    {
        ...
        Base.NetworkInfo = new RagdollData(Server.Host.ReferenceHub, handler,
            type, new RelativePosition(position), rotation, nickname, NetworkTime.time);
    ...
    public string Name
    {
        ...
        set => Base.NetworkInfo = new RagdollData(Base.Info.OwnerHub, Base.Info.Handler,
            Role, new RelativePosition(Position), Rotation, value, Base.Info.CreationTime, Base.Info.Serial);
    }
    ...
    public RoleTypeId Role
    {
        ...
        set => Base.NetworkInfo = new RagdollData(Base.Info.OwnerHub, Base.Info.Handler,
            value, new RelativePosition(Position), Rotation, Name, Base.Info.CreationTime, Base.Info.Serial);
    }
    ...
    public Player Owner
    {
        ...
        set
        {
            ...
            Base.NetworkInfo = new RagdollData(value.ReferenceHub, info.Handler, info.RoleType,
                info.StartRelativePosition, info.StartRelativeRotation, info.Nickname, info.CreationTime, info.Serial);
        }
    }
    ...
    private void SetupActions()
    {
        OnPositionUpdate += () =>
        {
            Base.NetworkInfo = new RagdollData(Base.Info.OwnerHub, Base.Info.Handler,
                Role, new RelativePosition(Position), Rotation, Name, Base.Info.CreationTime, Base.Info.Serial);
        };
