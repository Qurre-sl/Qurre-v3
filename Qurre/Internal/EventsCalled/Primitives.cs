    ...
    private static IEnumerator<float> ToysUpdate()
    {
        while (true)
        {
            foreach (Primitive prim in Primitive.NonStaticPrims)
                try
                {
                    var method = prim.Base.GetType().GetMethod("UpdatePositionServer", 
                        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                    method.Invoke(prim.Base, null);
                }
                catch
                {
            ...
            foreach (LightPoint light in Map.Lights)
                try
                {
                    var method = light.Base.GetType().GetMethod("UpdatePositionServer", 
                        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                    method.Invoke(light.Base, null);
                }
                catch
                {
                    // may flood, so disabled
            ...
            foreach (ShootingTarget shoot in Map.ShootingTargets)
                try
                {
                    var method = shoot.Base.GetType().GetMethod("UpdatePositionServer", 
                        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                    method.Invoke(shoot.Base, null);
                }
                catch
                {
                    // may flood, so disabled
            ...
            foreach (Speaker speaker in Map.Speakers)
                try
                {
                    var method = speaker.Base.GetType().GetMethod("UpdatePositionServer", 
                        System.Reflection.BindingFlags.NonPublic | System.Reflection.BindingFlags.Instance);
                    method.Invoke(speaker.Base, null);
                }
                catch
                {
                    // may flood, so disabled
