...
{
    private static readonly AccessTools.FieldRef<Scp079TierManager, int> AccessTierRef =
        AccessTools.FieldRefAccess<Scp079TierManager, int>("_accessTier");
    ...
    private static bool Call(Scp079TierManager __instance, ref int value)
    {
        try
        {
            if (AccessTierRef(__instance) == value)
                return false;
