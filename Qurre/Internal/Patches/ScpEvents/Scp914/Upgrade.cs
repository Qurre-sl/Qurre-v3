...
internal static class Upgrade
{
    ...
    private static readonly MethodInfo ProcessPlayerMethod =
        AccessTools.Method(typeof(Scp914Upgrader), "ProcessPlayer");

    private static readonly MethodInfo ProcessPickupMethod =
        AccessTools.Method(typeof(Scp914Upgrader), "ProcessPickup");
    ...
    private static void Invoke(Collider[] intake, Scp914Mode mode, Scp914KnobSetting setting)
    {
        ...
        try
        {
            ...
            foreach (Player pl in ev.Players)
                ProcessPlayerMethod.Invoke(null, new object[] { pl.ReferenceHub, flag, heldOnly, setting });

            foreach (ItemPickupBase item in ev.Items)
                ProcessPickupMethod.Invoke(null, new object[] { item, upgradeDropped, setting });
