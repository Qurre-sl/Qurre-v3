...
{
    private static readonly AccessTools.FieldRef<ServerRoles, ReferenceHub> HubRef = 
        AccessTools.FieldRefAccess<ServerRoles, ReferenceHub>("_hub");
    ...
    private static bool Call(ServerRoles __instance, ref UserGroup group)
    {
        try
        {
            Player? player = HubRef(__instance).GetPlayer();
