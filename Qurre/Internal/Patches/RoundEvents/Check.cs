...
internal static class Check
{
    ...
    private static IEnumerable<CodeInstruction> Call(IEnumerable<CodeInstruction> instr)
    {
        ...
        foreach (var code in codes.Select((x, i) => new { Value = x, Index = i }))
        {
            ...
            if (code.Value.operand is MethodBase { Name: "_ProcessServerSideCode" })
    ...
    private static readonly Type FriendlyFireConfigType = AccessTools.TypeByName("FriendlyFireConfig");

    private static readonly AccessTools.FieldRef<bool> PauseDetectorRef =
        AccessTools.StaticFieldRefAccess<bool>(AccessTools.Field(FriendlyFireConfigType, "PauseDetector"));
    
    private static readonly MethodInfo RpcShowRoundSummaryMethod =
        ReflectionHelper.GetMethod(typeof(RoundSummary), "RpcShowRoundSummary");
    
    private static readonly MethodInfo RpcDimScreenMethod =
        ReflectionHelper.GetMethod(typeof(RoundSummary), "RpcDimScreen");
    ...
    private static IEnumerator<float> ProcessServerSide(RoundSummary instance)
    {
        ...
        while (instance != null)
        {
            ...
            RpcShowRoundSummaryMethod.Invoke(instance, new object[]
            {
            ...
            RpcDimScreenMethod.Invoke(instance, null);
